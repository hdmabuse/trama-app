...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(false);
    if (res?.error) setError("Email ou senha inválidos, ou conta desativada.");
    ...
    <div className="min-h-screen bg-stone-50 flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm border border-stone-200 p-10">
        <div className="text-center mb-10">
          ...
          <p className="text-sm text-stone-400">Pesquisa qualitativa que se compõe</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-stone-500 mb-1.5">Email</label>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} required
              className="w-full px-3 py-2.5 rounded-lg border border-stone-200 text-sm focus:outline-none focus:ring-2 focus:ring-trama-500/30 focus:border-trama-500 transition" placeholder="seu@email.com" />
          </div>
          <div>
        ...
        <div className="text-center mt-6 space-y-2">
          <a href="/cadastro" className="block text-sm text-trama-500 font-medium">Criar conta grátis</a>
          <a href="/constituicao" className="block text-xs text-stone-400 hover:text-stone-600">Constituição ética</a>
        </div>
        <p className="text-center text-xs text-stone-400 mt-6">trama.app.br</p>
      </div>
    </div>
  );
}
