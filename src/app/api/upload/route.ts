...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (file.size > MAX_SIZE) {
      return NextResponse.json({ error: `Arquivo excede o limite de 50MB (${(file.size / 1024 / 1024).toFixed(1)}MB)` }, { status: 400 });
    }
    ...
    if (!mapped) {
      const allowed = Object.values(MIME_MAP).map(m => m.type).filter((v, i, a) => a.indexOf(v) === i);
