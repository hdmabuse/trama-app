...
const DICAS = [
  "Você pode codificar o mesmo trecho com múltiplos códigos — a realidade é multidimensional.",
  "Nomes descritivos funcionam melhor que avaliativos. 'Menciona custo' é mais útil que 'Reclama de caro'.",
  "Temas não são categorias — são respostas às suas perguntas de pesquisa.",
  "O memo é parte da análise, não margem do dado. Registre seu raciocínio interpretativo.",
  "Importe todos os documentos antes de codificar — padrões ficam mais visíveis com o material completo.",
  "O código que você escolhe já é uma interpretação. Não existe código neutro.",
  "Reflexividade não é confissão — é método. Registre por que você nomeou assim.",
  "A teia mostra conexões que o texto linear esconde. Use-a para revisar sua análise.",
  "Renomear um código muda o enquadramento de todos os trechos marcados com ele.",
  "Dois pesquisadores lendo o mesmo trecho podem codificá-lo diferente — e ambos podem estar certos.",
];

function getDicaDia(): string {
  return DICAS[Math.floor(Date.now() / 86400000) % DICAS.length];
}
...
export function DashboardClient({ projects }: { projects: Project[] }) {
  ...
  const [tourDismissed, setTourDismissed]     = useState(true); // começa true para evitar flash
  ...
  const dica       = getDicaDia();

  // Verificar localStorage uma única vez no mount
  useEffect(() => {
  ...
  return (
    <main className="flex-1 overflow-y-auto bg-stone-50">
      ...
      {/* ─── Banner de boas-vindas + tour (some após primeiro uso) ────── */}
      {!tourDismissed && (
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/75">
                  Faça o tour de introdução para entender como a análise reflexiva funciona.
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <button
                ...
                className="text-white/60 hover:text-white/90 text-xl leading-none transition"
      ...
      <div className="p-6 md:p-8">
        ...
        <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            {/* Botão de tour — sempre visível */}
            <button
              onClick={openTour}
              title="Tour de introdução"
        ...
        {limit !== Infinity && (
          <div className="mb-5 bg-white rounded-lg border border-stone-200 px-4 py-3 flex items-center gap-4">
            <span className="text-xs text-stone-500">Projetos: {projects.length}/{limit}</span>
            <div className="flex-1 bg-stone-100 rounded-full h-1.5 overflow-hidden">
              <div className={`h-full rounded-full transition-all ${barColor}`}
                style={{ width: `${Math.min(pct, 100)}%` }} />
            </div>
            {pct >= 80 && (
              <Link href="/cadastro" className="text-xs text-trama-500 font-medium shrink-0">Upgrade</Link>
            )}
          </div>
        )}
        ...
        {focal && (
          <div className="bg-white rounded-xl border border-stone-200 p-5 mb-5">
            ...
            <div className="flex items-start gap-4">
                ...
                {focal._count.documents > 0 && (
                  <>
                    ...
                    <p className="text-xs text-stone-400 mb-3">
                      {focal._count.documents} docs · {focal._count.codes} códigos
                    </p>
                  </>
                )}
              ...
              <div className="hidden sm:flex flex-col items-center justify-center w-20 h-16 border border-dashed border-stone-200 rounded-lg">
        ...
        <div className="grid grid-cols-3 gap-3 mb-5">
          {[
            ...
            { label: "Códigos",    value: totalCodes },
          ].map(s => (
        ...
        <div className="bg-white border-l-4 border-amber-400 rounded-r-xl px-4 py-3 mb-6 text-sm text-stone-600">
          <span className="font-medium text-stone-800">💡 Dica do dia: </span>
          {dica}
        </div>
        ...
        {projects.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h2 className="text-lg font-medium text-stone-800 mb-2">Nenhum projeto ainda</h2>
            <p className="text-sm text-stone-400 max-w-xs mb-6">
              Crie seu primeiro projeto para começar a organizar suas entrevistas e análises.
            </p>
            ...
            {/* Tour em destaque no empty state */}
            <button onClick={openTour}
              className="flex items-center gap-2 px-4 py-2 text-sm text-trama-500 hover:text-trama-700 border border-trama-200 hover:border-trama-400 bg-trama-50 hover:bg-trama-100 rounded-lg transition">
              <IconTour />
              Ver tour de introdução
            </button>
          </div>
        )}
